...
initializeGates <- function(gateNames) {
  for (gateName in gateNames) {
    flowWorkspace::gs_pop_add(
      gs=gatingset,
      gate=getGate(gateName),
      parent=getGateParent(gateName)
    )
  }
}

initializeGates(c("boundary", "nonNoise", "beadsA", "beadsB"))
...
initializeGates(c("cd24pos", "cd24neg", "cd24mid", "cd24hi"))
...
```{r anovaSummaryGraphs, echo=TRUE, results="markup", include=TRUE, warning=TRUE, message=TRUE}

summaryPlotting <- normalizedExperiment
summaryPlotting[["cd24midRelativeTo786"]] <- 1
summaryPlotting[["cd24hiRelativeTo786"]]  <- 1
summaryPlotting[["cd24posRelativeTo786"]] <- 1

for (n in summaryPlotting[["n"]]) {
  for (dilution in summaryPlotting[["dilution"]]) {
    correspondingData <- summaryPlotting %>%
      dplyr::filter(cellLine == "786-O", dilution == dilution) %>%
      dplyr::summarise(
        "cd24mid" = BiocGenerics::mean(cd24mid * cd24midTp),
        "cd24hi"  = BiocGenerics::mean(cd24hi * cd24hiTp),
        "cd24pos" = BiocGenerics::mean(cd24pos * cd24posTp)
      )

    summaryPlotting[["cd24midRelative"]] <-
      (summaryPlotting[["cd24mid"]] * summaryPlotting[["cd24midTp"]]) /
      correspondingData[["cd24mid"]]

    summaryPlotting[["cd24hiRelative"]] <-
      (summaryPlotting[["cd24hi"]] * summaryPlotting[["cd24hiTp"]]) /
      correspondingData[["cd24hi"]]

    summaryPlotting[["cd24posRelative"]] <-
      (summaryPlotting[["cd24pos"]] * summaryPlotting[["cd24posTp"]]) /
      correspondingData[["cd24pos"]]
  }
}

summaryPlotting <- summaryPlotting %>%
  dplyr::group_by(cellLine) %>%
  dplyr::summarise(
    "cd24midRelativeAvg" = BiocGenerics::mean(cd24midRelative),
    "cd24hiRelativeAvg"  = BiocGenerics::mean(cd24hiRelative),
    "cd24posRelativeAvg" = BiocGenerics::mean(cd24posRelative),
    "cd24midRelativeSem" = BiocGenerics::sd(cd24midRelative) / dplyr::n(),
    "cd24hiRelativeSem"  = BiocGenerics::sd(cd24hiRelative) / dplyr::n(),
    "cd24posRelativeSem" = BiocGenerics::sd(cd24posRelative) / dplyr::n(),
    .groups="drop"
  )

ggplot2::ggplot(
  data=summaryPlotting,
  mapping=ggplot2::aes(
    x=cellLine,
    y=cd24posRelativeAvg
  )
) +
  ggplot2::labs(
    title="",
  ...
  ggplot2::geom_errorbar(
    mapping=ggplot2::aes(
      ymin=cd24posRelativeAvg - cd24posRelativeSem,
      ymax=cd24posRelativeAvg + cd24posRelativeSem),
    width=0.5,
    size=1,
    show.legend=FALSE)
